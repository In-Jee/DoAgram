function [re_dec_azi re_dec_ev re_time re_freq]=DoAgram_de_TFscan(filename,Scan_Time_range,Scan_Freq_range,Alpha_threshold,FIG)
% [re_dec_azi re_dec_ev re_time re_freq]=DoAgram_de_TFscan(filename,Scan_Time_range,Scan_Freq_range,Alpha_threshold,FIG)
...
% 
% - Code description
% This is DoAgram decoding code from a DoAgram image w/ TF-scan method.
...
%    f=t*4000/5;
%    alpha_thres=0.2;
%    DoAgram_de_TFscan('decoding_test.png',t,f,alpha_thres,1);
% 
% - Example2
%    t=[0:0.1:4.9];
%    f=t*4000/5;
%    alpha_thres=0.2;
%    DoAgram_de_TFscan('decoding_test.png',t,f,alpha_thres,2);
%
...
% Filtering
for i=1:t_len
re_dec_azi(i)=dec_az(nf_freq(i),ntt(i));
re_dec_ev(i)=dec_ev(nf_freq(i),ntt(i));
re_alpha_data(i)=DoAgram_alpha(nf_freq(i),ntt(i));
end

re_dec_azi(find(re_alpha_data<Alpha_threshold))=NaN;
re_dec_ev(find(re_alpha_data<Alpha_threshold))=NaN;
ttext=num2str(tt');
ttext_f=num2str(f_freq');
...
ii=1:length(ntt);
for i=ii
    scatter3(dec_az(nf_freq(i),ntt(i)),dec_ev(nf_freq(i),ntt(i)),f_freq(i),DoAgram_alpha(nf_freq(i),ntt(i))*100+1,col(i,:),'fill')
    text(dec_az(nf_freq(i),ntt(i))+1,dec_ev(nf_freq(i),ntt(i))+10,f_freq(i),[ttext_f(i,:) ' Hz'],'fontsize',12);
if i==1
    colormap(jet(f_len))
end
...
figure()
col=jet(f_len);
scatter3(re_dec_azi,re_dec_ev,f_freq,re_alpha_data*100+1,col,'fill')
text(re_dec_azi,re_dec_ev,f_freq,ttext,'fontsize',8)
axis equal;xlim([-180 180]);ylim([-90 90]);box on;view([0 90]);
caxis([1-0.5 length(f)+0.5])
colormap(jet(length(f)))
colorbar('ticks',[1:length(f)],'TickLabels',num2cell(f_freq))
