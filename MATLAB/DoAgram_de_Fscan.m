function [re_dec_azi re_dec_ev re_time re_freq]=DoAgram_de_Fscan(filename,Scan_Time_range,Scan_Freq_range,Alpha_threshold,FIG)
% [re_dec_azi re_dec_ev re_time re_freq]=DoAgram_de_Fscan(filename,Scan_Time_range,Scan_Freq_range,Alpha_threshold,FIG)
...
% 
% - Code description
% This is DoAgram decoding code from a DoAgram image w/ F-scan method.
...
% Colorbar indicates the scan time range.
%
% - Example1
%    f=[[4280:10:4640] [5050:10:5530] [5920:10:6230]];
%    t=[2.0 2.9];
%    alpha_thres=0.2;
%    DoAgram_de_Fscan('decoding_test.png',t,f,alpha_thres,1);
% 
% - Example2
%    f=[[4280:10:4640] [5050:10:5530] [5920:10:6230]];
%    t=[1.0 2.0 2.9 3.5];
%    alpha_thres=0.2;
%    DoAgram_de_Fscan('decoding_test.png',t,f,alpha_thres,2);
%
...
ttext=num2str(reshape(repmat((tt),f_len,1),f_len*t_len,1));
...
col=jet(t_len);
for i=nf_freq
scatter3(dec_az(i,ntt),dec_ev(i,ntt),tt,DoAgram_alpha(i,ntt)*100+1,col,'fill')
if i==1
    colormap(jet(f_len))
end
      colormap(jet(length(t)))
      colorbar('ticks',[1:length(t)],'TickLabels',num2cell(tt))
      axis equal;xlim([-180 180]);ylim([-90 90]);box on;view([0 90]);
if f_freq(end)-f_freq(1)>0
      caxis([1-0.5 length(t)+0.5])
end
set(gca,'xtick',[-180:30:180],'ytick',[-90:30:90])
xlabel('Azimuth angle, deg');ylabel('Elevation angle, deg');
title(['Frequency index=' num2str(DoAgram_freq(i+1)) ' Hz'])
pause(0.01)
end
% ----------------------------------------------------------------------
...
col2=[];
for i=1:t_len
col2=[col2; repmat(col(i,:),f_len,1)];
end
scatter3(re_dec_azi,re_dec_ev,repmat(f_freq,1,t_len),re_alpha_data*100+1,col2,'fill')
text(re_dec_azi,re_dec_ev,repmat(f_freq,1,t_len),ttext,'fontsize',8)
        if t_len>f_len
            text_len=f_len;
        else
            text_len=t_len;
...
re_freq=repmat(f_freq,1,t_len)';
re_time=str2num(ttext);
